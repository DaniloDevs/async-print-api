  ...
  app.withTypeProvider<ZodTypeProvider>().post(
    "/events/:id/status",
    {
      schema: {
        ...
        tags: ["Events"],
        description:
          "Toggles the operational state of a specific event identified by its ID. Updates the `active` flag to enable or disable the event's availability and visibility in the system.",
        params: z.object({ id: z.string().describe("The unique identifier of the event") }),
        body: z.object({
          active: z
            .boolean()
            .describe("Determines if the event is live (true) or disabled (false)"),
        }),
        response: {
          200: z.void(),
          404: z.object({

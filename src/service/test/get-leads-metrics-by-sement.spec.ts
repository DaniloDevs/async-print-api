...
describe('Get leads metrics by segment (Service)', () => {
   ...
   describe("Successful cases", () => {
      ...
      it("should count only leads with intent to study next year per technical", async () => {
         await leadRepository.create(
            makeLead({ technicalInterest: "INF", intendsToStudyNextYear: true, eventId: event.id })
         );
         await leadRepository.create(
            makeLead({ technicalInterest: "INF", intendsToStudyNextYear: false, eventId: event.id })
         );
      ...
      it("should return all technical interests present in leads", async () => {
         ...
         for (const technical of technicals) {
            await leadRepository.create(
               makeLead({ technicalInterest: technical, eventId: event.id })
            );
         }

         const result = await sut.execute({ eventId: event.id });

         expect(result.technical.map(t => t.technical))
            .toEqual(expect.arrayContaining(technicals));
      });
